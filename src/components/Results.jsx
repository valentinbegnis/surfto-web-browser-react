          ...
          {results?.map(({ link, title, description }, index) => (
            <div key={index} className='md:w-2/5 w-full'>
              ...
              <p className='break-words text-sm text-gray-500'>
                {description ? description : 'No description available for this website.'}
              </p>
            </div>
          ))}
        </div>
        ...
        <div className='flex flex-wrap justify-between space-y-6 sm:px-56 items-center'>
          {results?.map(({ links, source, title, published }, index) => (
            <div key={index} className='md:w-2/5 w-full'>
              ...
              <a href={links?.[0].href} target='_blank' rel='noreferrer' className='hover:uderline'>
    ...
    case '/videos':
      return (
        <div className='flex flex-wrap justify-center'>
          {results?.filter(video => video.additional_links?.[0].href.includes('www.youtube.com/watch?'))
            .map((video, index) => {
              return (
